<?php 
    ...
    if(isset($_POST['add-item'])){
        ...
        if($category == "null-category"){
            ...
            $queryAdd = "INSERT INTO inventory (item_name, category, quantity, price, expiration_datetime) VALUES ('$item_name', '$category', $quantity, $price, '$expiration_date');";
...
<body class="bg-[#121C20] flex flex-col">
    ...
    <div class="flex items-center gap-[60px] p-2">
        <div class="flex items-center gap-5">
            ...
            <div class="text-white border-[1px] border-[#626D6E] px-2 py-1 rounded-lg">
                <p class="text-sm"><?= $sum?> <?php
                 if($sum < 3500) {?> <span class="text-red-500">Total product</span>
                <?php } else {?><span class="text-green-500">Total product</span>
                <?php }?>
                </p>
            </div>
        </div>
    ...
    <div class="flex">
        ...
        <div class="w-72 h-[560px] bg-[#1A262D] flex flex-col items-center justify-center rounded-lg self-center ml-3 p-9">
            ...
            <form action="" method="post" class="flex flex-col gap-5" id="myForm">
                ...
                <label for=""><span class="text-[#BEBDBD]">Item name</span>
                <input class="w-full px-2 py-1 border-[1px] border-[#626D6E] bg-transparent rounded-lg text-white text-center text-sm" type="text" name="item-name" id="" placeholder="Item name" required>
                </label>

                <label for=""><span class="text-[#BEBDBD]">Categories</span>
                    <select class=" w-full bg-transparent text-center text-sm text-[#9CA3AF] border-[1px] border-[#626D6E] rounded-lg px-2 py-1" name="category" id="" required>
                ...
                <label for=""><span class="text-[#BEBDBD]">Quantity</span><input class="w-full px-2 py-1 border-[1px] border-[#626D6E] bg-transparent rounded-lg text-white  text-center text-sm" type="number"  placeholder="Quantity" name="quantity" required>
                </label>
            ...
            <label for="" ><span class="text-[#BEBDBD]">Prices</span><input class="w-full px-2 py-1 border-[1px] border-[#626D6E] bg-transparent rounded-lg text-white  text-center text-sm" type="text"  placeholder="Price"  name="price" required></label>

            <label for=""><span class="text-[#BEBDBD]">Expiration Date:</span><input class="px-2 py-1 border-[1px] border-[#626D6E] bg-transparent rounded-lg text-white  text-center text-sm"  type="datetime-local" id="expiration_datetime" name="expiration-date"required></label>
            ...
            <div>
                ...
                ?>
                    ...
                    <div class="min-w-[50%]">
                        ...
                        <p class="text-[#626D6E]">Stoked Product <?php if($row['quantity'] < 500){?><span class="text-red-500"><?=$row['quantity'];?> in stock</span></p>
                        <?php } 
                        else{
                            ?>
                        ...
                        <p class="text-white border-l-2 pl-2 border-[#23323B]">
                        <?php
                    
                        if ($current_date >= $row['expiration_datetime']) {
                            
                           ?>
                       
                       <span class="text-red-500">Exp: <?= $row['expiration_datetime'];?></span>  <?php } else{?>
                        <span class="text-[#BEBDBD]s">Exp: <?= $row['expiration_datetime'];?> </span> <?php }?>
                        </p>    
                    </div>
                    </div>
